...
class RandomForest(object):
    ...
    def fitting(self):
        ...
        for tree,samples,labels in zip(self.decision_trees,self.bootstraps_datasets, self.bootstraps_labels):
            tree.learn(samples,labels)
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                for i in range(len(self.bootstraps_datasets)):
                    ...
                    effective_vote = OOB_tree.classify(record)
                    votes.append(effective_vote)
                    idx = np.random.randint(0, len(votes))
                    y = np.append(y, votes[idx])
