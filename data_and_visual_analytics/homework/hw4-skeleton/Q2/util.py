...
def entropy(class_y):
    ...
    result = []
    for i in set_y:
        p_i = len([x for x in class_y if x == i])/len_y
        result.append(-1*p_i*np.log2([p_i])[0])
    entropy = sum(result)
    return entropy
...
def partition_classes(X, y, split_attribute, split_val):
    ...
    y = np.array(y)
    if isinstance(split_val, str) or isinstance(split_val, int):
...
def information_gain(previous_y, current_y):
    ...
    info_gain = 0.45915
    """
    H_prev = [entropy(h)*len(h)/len(previous_y) for h in previous_y]

    H_current = [entropy(h)*len(h)/len(previous_y) for h in current_y]

    info_gain = sum(H_prev) - sum(H_current)
