...
def get_cost(centroids, pixels, classes, ord=None):
    ...
    for i, v in enumerate(centroids):
        ...
        d = np.linalg.norm((centroid - pxls), axis=0, ord=ord)
        cost += np.sum(d)
    return cost
...
def my_kmedoids(pixels, K, initial_centroids=False, RUNS=200, ord=None):
    ...
    if not initial_centroids:
        centroids_k_plus_plus = k_plus_plus(pixels, K)
        distances = get_distances(centroids_k_plus_plus, pixels, ord=None)
        idxs = list(get_closest_idxs(distances))
        idxs.sort()
    ...
    idxs_list = [idxs]
    # Assign all points to the closest cluster using the euclidean distance
    # as measure
    classes = get_classes(centroids, pixels)
    # Calculate the initial total cost which is the sum of all distances.
    best_cost = get_cost(centroids, pixels, classes)
    runs = 0
    while runs < RUNS:
        # Randomly choose a new cluster center to replace one of the
        # existing cluster centers and check the cluster set has not been used before
        new_idxs = list(get_initial_idxs(pixels, K))
        new_idxs.sort()
        # if new_idxs in idxs_list:
        #   continue
        idxs_list.append(new_idxs)
        # Calculate cost of new centroids.  Update best cluster centers
        # if new cost < previous cost
        new_centroids = pixels[new_idxs].reshape(K, 3)
        new_classes = get_classes(new_centroids, pixels)
        new_cost = get_cost(new_centroids, pixels, new_classes)
